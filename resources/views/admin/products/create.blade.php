...
>
  ...
  <div class="bg-white shadow rounded-xl p-6">
                <h2 class="text-xl font-semibold text-gray-800 mb-4">Basic Product Info</h2>
                <form id="product-form" method="POST" action="{{ route('admin.products.store') }}" enctype="multipart/form-data" class="space-y-4">
    @csrf
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <input type="number" name="discount_price" class="w-full border rounded px-3 py-2" step="0.01" min="0"> 
        </div>
        ...
        <div>
            ...
            <input type="number" name="rating" class="w-full border rounded px-3 py-2" max="5" required>
        </div>
...
<div class="bg-white shadow rounded-xl p-6">
    ...
    <button type="button" onclick="addSize()" 
        class="px-4 py-2 mt-5 bg-primary text-white rounded hover:bg-emerald-600">
        + Add Size
    </button>
</div>
    ...
    <div class="bg-white shadow rounded-xl p-6">
        ...
        <div>
            ...
            <div id="additional-preview" class="flex gap-2 flex-wrap cursor-pointer" onclick="document.getElementById('additional-images').click()">
                <div class="w-24 h-24 border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center hover:border-primary">
    ...
    <div class="flex justify-end space-x-3 mt-4">
        <button type="submit" class="px-6 py-2 bg-primary text-white rounded hover:bg-emerald-600">Add Product</button>
    </div>
</form>
...
document.addEventListener('DOMContentLoaded', function() {
            ...
            if (!filesArray.some(f => f.name === file.name)) {
                ...
                reader.onload = function(ev) {
                    ...
                    div.innerHTML = `
                        ...
                        <button type="button" class="absolute top-0 right-0 bg-red-500 text-white rounded-full w-5 h-5 flex items-center justify-center" onclick="removeAdditionalImage(this, '${file.name}')">
                            <i class="fas fa-times text-xs"></i>
                        </button>`;
    ...
    window.removeAdditionalImage = function(el, filename) {
    ...
    const html = `
        <div class="flex gap-2 items-center mb-2">
            ...
            <input type="number" name="sizes[${sizeIndex}][price]" 
                ...
                placeholder="Auto price" readonly>

            <button type="button" onclick="removeSize(this)" class="text-red-500 font-bold px-2">X</button>
        </div>`;
...
function recalcPrices() {
    ...
    const rows = document.querySelectorAll('#sizesContainer .size-input');
...
document.getElementById('product-form').addEventListener('submit', function(e) {
    const defaultPrice = parseFloat(document.querySelector('input[name="price"]').value); // price per kg
    const defaultWeight = parseFloat(document.querySelector('input[name="weight"]').value) || 1; // default weight if needed
